...
class FormReader<bool> : public FormReaderBase<FormReader<bool>> {
 public:
  ...
  template <class Func>
  static bool GetFunctionForForm(const DIEReader&, uint8_t form,
                                 Func func) {
    switch (form) {
      case DW_FORM_flag:
...
class FormReader<void> : public FormReaderBase<FormReader<void>> {
 public:
  ...
  FormReader(const DIEReader& reader, StringPiece data, void*)
      : Base(reader, data) {}
...
class FixedAttrReader {
 public:
  ...
  template <size_t N>
  FixedAttrReader(DIEReader*, const DwarfAttribute (&attributes)[N]) {
